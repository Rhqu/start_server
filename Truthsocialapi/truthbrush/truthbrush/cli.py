...
import datetime
from .api import Api
from .investment_client import InvestmentClient
...
# Investment sector commands

@cli.command()
def sectors():
    """List all available investment sectors."""
    for key in InvestmentClient.list_sectors():
        info = InvestmentClient.get_sector_info(key)
        print(f"{key}: {info.name} ({info.name_de})")


@cli.command()
@click.argument("sector_name")
@click.option("--limit", default=50, help="Max posts to return", type=int)
@click.option("--min-followers", default=5000, help="Min author followers", type=int)
@click.option("--min-engagement", default=10, help="Min total engagement", type=int)
def sector(sector_name: str, limit: int, min_followers: int, min_engagement: int):
    """Get popular posts from an investment sector."""
    client = InvestmentClient()
    try:
        posts = client.get_popular_sector_posts(
            sector_name, limit, min_followers, min_engagement
        )
        for post in posts:
            print(json.dumps(asdict(post)))
    except ValueError as e:
        print(f"Error: {e}")


@cli.command("invest-search")
@click.argument("query")
@click.option("--limit", default=50, help="Max posts to return", type=int)
@click.option("--min-followers", default=0, help="Min author followers", type=int)
def invest_search(query: str, limit: int, min_followers: int):
    """Search investment-related posts with custom query."""
    client = InvestmentClient()
    posts = client.custom_query(query, limit, min_followers)
    for post in posts:
        print(json.dumps(asdict(post)))
