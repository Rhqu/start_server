...
class RateLimitException(Exception):
    """Raised when rate limited by Truth Social or Cloudflare"""
    pass
...
class Api:
    ...
    def _get(self, url: str, params: dict = None) -> Any:
        ...
        # Check for rate limiting or blocking before parsing JSON
        if resp.status_code in (429, 403):
            response_text = resp.text.lower()
            if "rate limit" in response_text or "1015" in resp.text:
                logger.error(f"Rate limited by Cloudflare (status {resp.status_code})")
                raise RateLimitException("Rate limited by Cloudflare. Wait and retry.")
            if "blocked" in response_text or "banned" in response_text:
                logger.error(f"Blocked by Cloudflare (status {resp.status_code})")
                raise CFBlockException("Blocked by Cloudflare")
            logger.error(f"Request failed with status {resp.status_code}")
            raise LoginErrorException(f"Request failed with status {resp.status_code}")
    ...
    def hashtag(
        ...
        while num_results < limit:
            ...
            # Filter out empty results and ensure they are dicts
            results = [value for value in resp if value and isinstance(value, dict)]
            if not results:
                break
